...
from ombootstrap.constants import Arch, CHROOT_PATHS, GCC_HOSTSPECS
from ombootstrap.distro.distro import (
    get_base_distro,
    get_kupfer_local,
    RepoInfo,
)
from ombootstrap.exec.cmd import (
    FileDescriptor,
    run_root_cmd,
    generate_env_cmd,
...
from ombootstrap.generator import generate_makepkg_conf
from ombootstrap.utils import (
    mount,
    umount,
    check_findmnt,
    log_or_exception,
...
class AbstractChroot(Protocol):
    ...
    def initialize(
        self, reset: bool = False, fail_if_initialized: bool = False
    ):
    ...
    def try_install_packages(
        self, packages: list[str], refresh: bool, allow_fail: bool
...
class Chroot(AbstractChroot):
    ...
    def __init__(
        ...
        self.name = name
        self.arch = arch
        self.path = path_override or os.path.join(
            config.get_path("chroots"), name
        )
        self.copy_base = (
            copy_base if copy_base is not None else self._copy_base
        )
        self.extra_repos = deepcopy(extra_repos)
        self.base_packages = base_packages.copy()
        if self.name.startswith(BASE_CHROOT_PREFIX) and set(
            get_kupfer_local(self.arch).repos
        ).intersection(set(self.extra_repos)):
            raise Exception(
                f"Base chroot {self.name} had local repos specified: {self.extra_repos}"
            )

    def create_rootfs(
        self, reset: bool, pacman_conf_target: str, active_previously: bool
    ):
        raise NotImplementedError()
    ...
    def initialize(
        ...
        if self.initialized and not reset:
            ...
            if fail_if_initialized:
                raise Exception(
                    f"Chroot {self.name} ({self.uuid}) is already initialized, this seems like a bug"
                )
            logging.debug(
                f"Base chroot {self.name} ({self.uuid}) already initialized"
            )
            return
        ...
        if not (os.path.ismount(self.path) or check_findmnt(self.path)):
            ...
            logging.debug(
                f"Bind-mounting {self.name} ({self.uuid}) over itself"
            )  # needed for pacman space calculation
    ...
    def mount(
        ...
        def log_or_exc(msg):
            log_or_exception(
                strict_cache_consistency, msg, log_level=logging.ERROR
            )
        ...
        if check_findmnt(absolute_destination):
            if pseudo_absolute not in self.active_mounts:
                same_source = False
                if any(opt in options for opt in ("bind", "rbind")):
                    try:
                        same_source = os.path.samefile(
                            absolute_source, absolute_destination
                        )
                    except OSError:
                        pass
                elif (
                    pseudo_absolute in BASIC_MOUNTS
                    and fs_type == BASIC_MOUNTS[pseudo_absolute]["type"]
                    and fs_type in ("proc", "sysfs", "devtmpfs", "devpts", "tmpfs")
                ):
                    # Kernel pseudo-filesystems have symbolic sources, not paths.
                    mounted = run_root_cmd(
                        [
                            "findmnt", "--noheadings", "--raw", "--first-only",
                            "--mountpoint", absolute_destination,
                            "--output", "FSTYPE,FSROOT",
                        ],
                        capture_output=True,
                    )
                    if mounted.returncode == 0:
                        same_source = mounted.stdout.decode().split() == [fs_type, "/"]
                if fail_if_mounted or strict_cache_consistency or not same_source:
                    raise Exception(
                        f"{self.name}: Untracked mount at {absolute_destination}; "
                        f"cannot reuse it for {absolute_source}."
                    )
                logging.info(
                    f"{self.name}: Reusing existing mount for {pseudo_absolute}"
                )
                self.active_mounts.append(pseudo_absolute)
                atexit.register(self.deactivate)
            elif fail_if_mounted:
                raise Exception(
                    f"{self.name}: {absolute_destination} is already mounted"
                )
            logging.debug(
                f"{self.name}: {absolute_destination} already mounted. Skipping."
            )
        else:
            if pseudo_absolute in self.active_mounts:
                log_or_exc(
                    f"{self.name}: Mount {pseudo_absolute} was in active_mounts but not actually mounted. ({absolute_destination})"
                )
                self.active_mounts.remove(pseudo_absolute)
            ...
            result = mount(
                ...
                options=options,
                fs_type=fs_type,
                register_unmount=False,
            )
            if result.returncode != 0:
                raise Exception(
                    f"{self.name}: failed to mount {absolute_source} to {absolute_destination}"
                )
            logging.debug(
                f"{self.name}: {absolute_source} successfully mounted to {absolute_destination}."
            )
    ...
    def umount(self, relative_path: str):
        ...
        if (
            result.returncode == 0
            and make_abs_path(relative_path) in self.active_mounts
        ):
            self.active_mounts.remove(make_abs_path(relative_path))
        return result
    ...
    def activate(self, fail_if_active: bool = False):
        ...
        for dst, opts in BASIC_MOUNTS.items():
            self.mount(
                ...
                fail_if_mounted=fail_if_active,
            )
        self.active = True
    ...
    def deactivate(
        self, fail_if_inactive: bool = False, ignore_rootfs: bool = False
    ):
        if not self.active:
            if fail_if_inactive:
                raise Exception(
                    f"Chroot {self.name} not activated, can't deactivate!"
                )
        self.umount_many(
            [
                mnt
                for mnt in self.active_mounts
                if mnt not in ["/", "/boot"] or not ignore_rootfs
            ]
        )
        self.active = False
    ...
    def run_cmd(
        ...
        if not self.active and fail_inactive:
            raise Exception(
                f"Chroot {self.name} is inactive, not running command! Hint: pass `fail_inactive=False`"
            )
        ...
        if self.arch != native and "QEMU_LD_PREFIX" not in outer_env:
            ...
            outer_env |= {
                "QEMU_LD_PREFIX": f"/usr/{GCC_HOSTSPECS[native][self.arch]}"
            }
        env_cmd = generate_env_cmd(inner_env) if inner_env else []

        if not isinstance(script, str) and isinstance(script, list):
            script = flatten_shell_script(
                script, shell_quote_items=False, wrap_in_shell_quote=False
            )
        if cwd:
            script = f"cd {shell_quote(cwd)} && ( {script} )"
        if switch_user:
            inner_cmd = generate_cmd_su(
                script,
        ...
        cmd = flatten_shell_script(
            ["chroot", self.path] + env_cmd + inner_cmd, shell_quote_items=True
        )
    ...
    def write_makepkg_conf(
        self,
        target_arch: Arch,
        cross_chroot_relative: Optional[str],
        cross: bool = True,
    ) -> str:
        """
        ...
        """
        makepkg_cross_conf = generate_makepkg_conf(
            target_arch, cross=cross, chroot=cross_chroot_relative
        )
        filename = (
            "makepkg" + (f"_cross_{target_arch}" if cross else "") + ".conf"
        )
    ...
    def write_pacman_conf(
        self,
        check_space: Optional[bool] = None,
        in_chroot: bool = True,
        ...
        conf_text = get_base_distro(self.arch).get_pacman_conf(
            repos, check_space=check_space, in_chroot=in_chroot
        )
    ...
    def get_uid(self, user: Union[str, int]) -> int:
        ...
        if res.returncode or not res.stdout:
            raise Exception(
                f"chroot {self.name}: Couldnt detect uid for user {user}: {repr(res.stdout)}"
            )
    ...
    def add_sudo_config(
        self,
        config_name: str = "wheel",
        privilegee: str = "%wheel",
        password_required: bool = True,
    ):
        if "." in config_name:
            raise Exception(
                f"won't create sudoers.d file {config_name} since it will be ignored by sudo because it contains a dot!"
            )
        comment = (
            "# allow "
            + (
                f"members of group {privilegee.strip('%')}"
                if privilegee.startswith("%")
                else f"user {privilegee}"
            )
            + "to run any program as root"
            + ("" if password_required else " without a password")
        )
        line = privilegee + (
            " ALL=(ALL:ALL) ALL"
            if password_required
            else " ALL=(ALL) NOPASSWD: ALL"
        )
        root_write_file(
            self.get_path(f"/etc/sudoers.d/{config_name}"),
            f"{comment}\n{line}",
        )
    ...
    def try_install_packages(
        ...
        if refresh:
            results["refresh"] = self.run_cmd(
                "pacman -Syy --noconfirm", stderr=stderr
            )
