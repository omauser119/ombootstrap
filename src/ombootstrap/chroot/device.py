...
from ombootstrap.constants import Arch, BASE_PACKAGES
from ombootstrap.distro.repo import RepoInfo
from ombootstrap.distro.distro import get_kupfer_local, get_kupfer_https
from ombootstrap.exec.file import get_temp_dir, makedir, root_makedir
from ombootstrap.utils import check_findmnt
...
class DeviceChroot(BuildChroot):
    ...
    def create_rootfs(self, reset, pacman_conf_target, active_previously):
        ...
        if not self.copy_base:
            pacman_conf_target = os.path.join(
                get_temp_dir(register_cleanup=True), f"pacman-{self.name}.conf"
            )
            self.write_pacman_conf(
                in_chroot=False, absolute_path=pacman_conf_target
            )
            BaseChroot.create_rootfs(
                cast(BaseChroot, self),
                reset,
                pacman_conf_target,
                active_previously,
            )
        else:
            BuildChroot.create_rootfs(
                self, reset, pacman_conf_target, active_previously
            )
    ...
    def mount_rootfs(
        ...
        if self.active:
            raise Exception(
                f"{self.name}: Chroot is marked as active, not mounting a rootfs over it."
            )
        ...
        if not allow_overlay:
            ...
            if really_active:
                raise Exception(
                    f"{self.name}: Chroot has submounts active: {really_active}"
                )
            if os.path.ismount(self.path) or check_findmnt(self.path):
                raise Exception(
                    f"{self.name}: There is already something mounted at {self.path}, not mounting over it."
                )
            if os.path.exists(os.path.join(self.path, "usr/bin")):
                raise Exception(
                    f"{self.name}: {self.path}/usr/bin exists, not mounting over existing rootfs."
                )
        makedir(self.path)
