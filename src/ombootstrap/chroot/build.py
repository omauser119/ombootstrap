...
class BuildChroot(Chroot):
    ...
    def create_rootfs(
        ...
        if reset or not os.path.exists(self.get_path("usr/bin")):
            ...
            if base_chroot == self:
                raise Exception(
                    "base_chroot == self, bailing out. this is a bug"
                )
            ...
            if result.returncode != 0:
                raise Exception(
                    f"Failed to copy {base_chroot.name} to {self.name}"
                )

        else:
            logging.debug(f"{self.name}: Reusing existing installation")

        if set(get_kupfer_local(self.arch).repos).intersection(
            set(self.extra_repos)
        ):
            self.mount_packages()
        ...
        self.activate()
        self.try_install_packages(
            self.base_packages, refresh=True, allow_fail=False
        )
        ...
        # configure makepkg
        self.write_makepkg_conf(
            self.arch, cross_chroot_relative=None, cross=False
        )

        if active_previously:
            self.activate()

    def mount_crossdirect(
        self,
        native_chroot: Optional[Chroot] = None,
        fail_if_mounted: bool = False,
    ):
        """
        ...
        results = dict(
            native_chroot.try_install_packages(
        ...
        for target, source in {
            cc_path: gcc,
            target_lib_dir: "lib",
            target_include_dir: "usr/include",
        }.items():
            if not (os.path.exists(target) or os.path.islink(target)):
                logging.debug(f"Symlinking {source=} at {target=}")
                symlink(source, target)
        ld_so = os.path.basename(
            glob(
                f"{os.path.join(native_chroot.path, 'usr', 'lib', 'ld-linux-')}*"
            )[0]
        )
        ...
        if not os.path.islink(ld_so_target):
            ...
            logging.debug(
                f"ld-linux.so symlink already exists, skipping for {self.name}"
            )

        # TODO: find proper fix
        rustc = os.path.join(
            native_chroot.path, "usr/lib/crossdirect", target_arch, "rustc"
        )
        ...
        self.mount(
            native_chroot.path, "native", fail_if_mounted=fail_if_mounted
        )
        return native_mount

    def mount_crosscompile(
        self, foreign_chroot: Chroot, fail_if_mounted: bool = False
    ):
        mount_dest = os.path.join(
            CHROOT_PATHS["chroots"].lstrip("/"),
            os.path.basename(foreign_chroot.path),
        )
        return self.mount(
    ...
    def mount_ccache(
        self, user: str = "omarchy", fail_if_mounted: bool = False
    ):
        mount_source = os.path.join(config.get_path("ccache"), self.arch)
        mount_dest = os.path.join(
            f"/home/{user}" if user != "root" else "/root", ".ccache"
        )
    ...
    def mount_rust(
        ...
        for rust_dir in ["cargo", "rustup"]:
            ...
            mount_dest = os.path.join(
                f"/home/{user}" if user != "root" else "/root", f".{rust_dir}"
            )
            makedir(mount_source, user=uid)
            results.append(
                self.mount(
...
def get_build_chroot(
    arch: Arch, add_kupfer_repos: bool = True, **kwargs
) -> BuildChroot:
