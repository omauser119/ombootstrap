...
from ombootstrap.constants import CHROOT_PATHS
from ombootstrap.exec.file import remove_file
from ombootstrap.packages.cli import cmd_clean as cmd_clean_pkgbuilds
...
@click.argument(
    "paths", nargs=-1, type=click.Choice(["all"] + PATHS), required=False
)
@click.pass_context
def cmd_clean(
    ctx: click.Context,
    ...
    for path_name in PATHS:
        if query and not force:
            clear[path_name] = click.confirm(
                f"{'(Noop) ' if noop else ''}Clear {path_name}?"
            )
        if clear[path_name]:
            ...
            for file in os.listdir(dir):
                ...
                log(
                    f'{"Would remove" if noop else "Removing"} "{path_name}/{file}"'
                )
                if not noop:
