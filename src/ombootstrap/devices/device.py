...
from ombootstrap.distro.distro import get_kupfer_local
from ombootstrap.distro.package import LocalPackage
from ombootstrap.packages.pkgbuild import (
    Pkgbuild,
    _pkgbuilds_cache,
    discover_pkgbuilds,
    get_pkgbuild_by_path,
    init_pkgbuilds,
)
from ombootstrap.utils import read_files_from_tar, color_str

from .deviceinfo import DEFAULT_IMAGE_SECTOR_SIZE, DeviceInfo, parse_deviceinfo

DEVICE_DEPRECATIONS = {
    "a6plte": "msm8953-a6plte",
    "samsung-a6plte": "msm8953-a6plte",
...
class DeviceSummary(DictScheme):
    ...
    def nice_str(self, newlines: bool = False, colors: bool = False) -> str:
        ...
        package_path = (
            {"Package Path": self.package_path} if self.package_path else {}
        )
        fields = {
            "Device": self.name,
            "Description": self.description
            or f"[no package {'description' if self.package_name else 'associated (?!)'} and deviceinfo not parsed]",
            "Architecture": self.arch,
            "Package Name": self.package_name
            or "no package associated. PROBABLY A BUG!",
            **package_path,
        }
        return separator.join(
            [
                f"{color_str(name, bold=True, use_colors=colors)}: {value}"
...
class Device(DictScheme):
    ...
    def get_summary(self) -> DeviceSummary:
        ...
        description = (
            (self.package.description if self.package else "").strip()
            or (
                self.deviceinfo.get("name", "[No name in deviceinfo]")
                if self.deviceinfo
                else ""
            )
        ).strip()
    ...
    def parse_deviceinfo(
        self, try_download: bool = True, lazy: bool = True
    ) -> DeviceInfo:
        if not lazy or "deviceinfo" not in self or self.deviceinfo is None:
            # avoid import loop
            from ombootstrap.packages.build import (
                check_package_version_built,
            )

            is_built = check_package_version_built(
                self.package, self.arch, try_download=try_download
            )
            if not is_built:
                raise Exception(
                    f"device package {self.package.name} for device {self.name} couldn't be acquired!"
                )
            pkgs: dict[str, LocalPackage] = get_kupfer_local(
                arch=self.arch, in_chroot=False, scan=True
            ).get_packages()
            if self.package.name not in pkgs:
                raise Exception(
                    f"device package {self.package.name} somehow not in repos, this is a ombootstrap bug"
                )
            ...
            for path, f in read_files_from_tar(file_path, [deviceinfo_path]):
                if path != deviceinfo_path:
                    raise Exception(
                        f'Somehow, we got a wrong file: expected: "{deviceinfo_path}", got: "{path}"'
                    )
                with f as fd:
                    lines = fd.readlines()
                    assert lines
    ...
    def get_image_sectorsize(self, **kwargs) -> Optional[int]:
        ...
        return self.parse_deviceinfo(**kwargs).get(
            "rootfs_image_sector_size", None
        )
...
def check_devicepkg_name(name: str, log_level: Optional[int] = None):
    ...
    if not name.startswith("device-"):
        ...
        if log_level is not None:
            logging.log(
                log_level,
                f'invalid device package name "{name}": doesn\'t start with "device-"',
            )
    if name.endswith("-common"):
        ...
        if log_level is not None:
            logging.log(
                log_level,
                f'invalid device package name "{name}": ends with "-common"',
            )
    return valid
...
def parse_device_pkg(pkgbuild: Pkgbuild) -> Device:
    if len(pkgbuild.arches) != 1:
        raise Exception(
            f"{pkgbuild.name}: Device package must have exactly one arch, but has {pkgbuild.arches}"
        )
    ...
    if pkgbuild.repo != "device":
        logging.warning(
            f'device package {pkgbuild.name} is in unexpected repo "{pkgbuild.repo}", expected "device"'
        )
    name = pkgbuild.name
    prefix = "device-"
...
def sanitize_device_name(name: str, warn: bool = True) -> str:
    ...
    if replacement:
        warning += (
            f": Device has been renamed to {replacement}! Please adjust your profile config!\n"
...
def get_devices(
    pkgbuilds: Optional[dict[str, Pkgbuild]] = None, lazy: bool = True
) -> dict[str, Device]:
    ...
    if not use_cache:
        ...
        for pkgbuild in pkgbuilds.values():
            if not (
                pkgbuild.repo == "device"
                and check_devicepkg_name(pkgbuild.name, log_level=None)
            ):
                continue
...
def get_device(
    name: str,
    pkgbuilds: Optional[dict[str, Pkgbuild]] = None,
    lazy: bool = True,
    scan_all=False,
) -> Device:
    global _device_cache, _device_cache_populated
        ...
        if pkgbuilds:
            ...
            if lazy and pkgname in _pkgbuilds_cache:
                ...
                if not os.path.exists(
                    os.path.join(config.get_path("pkgbuilds"), relative_path)
                ):
                    logging.debug(
                        f'Exact device pkgbuild path "pkgbuilds/{relative_path}" doesn\'t exist, scanning entire repo'
                    )
                    return get_device(
                        name, pkgbuilds=pkgbuilds, lazy=lazy, scan_all=True
                    )
                pkgbuild = [
                    p
                    for p in get_pkgbuild_by_path(relative_path, lazy=lazy)
                    if p.name == pkgname
                ][0]
...
def get_profile_device(
    profile_name: Optional[str] = None, hint_or_set_arch: bool = False
):
    profile = config.enforce_profile_device_set(
        profile_name=profile_name, hint_or_set_arch=hint_or_set_arch
    )
