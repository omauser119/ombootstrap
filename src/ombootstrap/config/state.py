...
from ombootstrap.constants import DEFAULT_PACKAGE_BRANCH
...
CONFIG_DIR = appdirs.user_config_dir("ombootstrap")
CACHE_DIR = appdirs.user_cache_dir("ombootstrap")
CONFIG_DEFAULT_PATH = os.path.join(CONFIG_DIR, "ombootstrap.toml")
...
def sanitize_config(
    conf: dict[str, dict], warn_missing_defaultprofile=True
) -> dict[str, dict]:
    """checks the input config dict for unknown keys and returns only the known parts"""
    return merge_configs(
        conf_new=conf,
        conf_base={},
        warn_missing_defaultprofile=warn_missing_defaultprofile,
    )


def merge_configs(
    conf_new: Mapping[str, dict],
    conf_base={},
    warn_missing_defaultprofile=True,
) -> dict[str, dict]:
    """
    ...
    for outer_name, outer_conf in deepcopy(conf_new).items():
        ...
        if not (isinstance(outer_conf, (dict, DictScheme))):
            ...
            if outer_name == "profiles":
                if (
                    warn_missing_defaultprofile
                    and "default" not in outer_conf.keys()
                ):
                    logging.warning(
                        "Default profile is not defined in config file"
                    )
                ...
                for profile_name, profile_conf in outer_conf.items():
                    if not isinstance(profile_conf, (dict, Profile)):
                        if profile_name == "current":
                            ...
                            logging.warning(
                                f'Skipped key "{profile_name}" in profile section: only subsections and "current" allowed'
                            )
                        continue
                    ...
                    for key, val in profile_conf.items():
                        if key not in PROFILE_DEFAULTS:
                            logging.warning(
                                f'Skipped unknown config item "{key}" in profile "{profile_name}"'
                            )
                            continue
                        profile[key] = val
                ...
                for inner_name, inner_conf in outer_conf.items():
                    if inner_name not in CONFIG_DEFAULTS[outer_name].keys():
                        logging.warning(
                            f'Skipped unknown config item "{inner_name}" in section "{outer_name}"'
                        )
                        continue
...
def parse_file(config_file: str, base: dict = CONFIG_DEFAULTS) -> dict:
    ...
    """
    _conf_file = (
        config_file if config_file is not None else CONFIG_DEFAULT_PATH
    )
...
class ConfigLoadException(Exception):
    ...
    def __init__(
        self, extra_msg="", inner_exception: Optional[Exception] = None
    ):
...
class ConfigStateHolder:
    ...
    def __init__(
        self,
        file_conf_path: Optional[str] = None,
        runtime_conf={},
        file_conf_base: dict = {},
    ):
        """init a stateholder, optionally loading `file_conf_path`"""
        self.file = Config.fromDict(
            merge_configs(conf_new=file_conf_base, conf_base=CONFIG_DEFAULTS)
        )
        self.file_state = ConfigLoadState()
        self.runtime = RuntimeConfiguration.fromDict(
            CONFIG_RUNTIME_DEFAULTS | runtime_conf
        )
        self.runtime.arch = os.uname().machine
        self.runtime.script_source_dir = os.path.dirname(
    ...
    def try_load_file(self, config_file=None, base=CONFIG_DEFAULTS_DICT):
        ...
        try:
            self.file = Config.fromDict(
                parse_file(config_file=config_file, base=base), validate=True
            )
    ...
    def is_loaded(self) -> bool:
        ...
        return (
            self.file_state.load_finished and self.file_state.exception is None
        )

    def enforce_config_loaded(self):
        if not self.file_state.load_finished:
            m = "Config file wasn't even parsed yet. This is probably a bug in ombootstrap :O"
            raise ConfigLoadException(Exception(m))
        ex = self.file_state.exception
        if ex:
            if isinstance(ex, FileNotFoundError):
                ex = Exception(
                    "Config file doesn't exist. Try running `ombs config init` first?"
                )
            raise ex

    def get_profile(self, name: Optional[str] = None) -> Profile:
        name = name or self.file.profiles.current
        self._profile_cache = resolve_profile(
            name=name,
            sparse_profiles=self.file.profiles,
            resolved=self._profile_cache,
        )
        return self._profile_cache[name]

    def _enforce_profile_field(
        self,
        field: str,
        profile_name: Optional[str] = None,
        hint_or_set_arch: bool = False,
    ) -> Profile:
        # TODO: device
        profile_name = (
            profile_name
            if profile_name is not None
            else self.file.profiles.current
        )
        arch_hint = ""
        if not hint_or_set_arch:
            ...
            arch_hint = (
                " or specifiy the target architecture by passing `--arch` to the current command,\n"
                "e.g. `ombs packages build --arch x86_64`"
            )
            if not self.is_loaded():
                if not self.file_state.exception:
                    raise Exception(
                        f"Error enforcing config profile {field}: config hadn't even been loaded yet.\n"
                        "This is a bug in ombootstrap!"
                    )
                raise Exception(
                    f"Profile {field} couldn't be resolved because the config file couldn't be loaded.\n"
                    "If the config doesn't exist, try running `ombs config init`.\n"
                    f"Error: {self.file_state.exception}"
                )
        if profile_name and profile_name not in self.file.profiles:
            raise Exception(
                f'Unknown profile "{profile_name}". Please run `ombs config profile init`{arch_hint}'
            )
        profile = self.get_profile(profile_name)
        if field not in profile or not profile[field]:
            m = (
                f'Profile "{profile_name}" has no {field.upper()} configured.\n'
                f"Please run `ombs config profile init {profile_name}`{arch_hint}"
            )
            raise Exception(m)
        return profile
    ...
    def write(self, path=None):
        ...
        logging.info(
            f"{'Created' if new else 'Written changes to'} config file at {path}"
        )
    ...
    def update(
        self,
        config_fragment: dict[str, dict],
        warn_missing_defaultprofile: bool = True,
    ) -> bool:
        ...
        if (
            changed
            and "profiles" in config_fragment
            and self.file.profiles.toDict() != config_fragment["profiles"]
        ):
