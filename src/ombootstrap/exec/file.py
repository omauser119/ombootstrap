...
from ombootstrap.utils import get_user_name, get_group_name


def try_native_filewrite(
    path: str, content: Union[str, bytes], chmod: Optional[str] = None
) -> Optional[Exception]:
...
def root_check_exists(path):
    return (
        os.path.exists(path)
        or run_root_cmd(["[", "-e", path, "]"]).returncode == 0
    )
...
def write_file(
    ...
    if mode:
        if not mode.isnumeric():
            raise Exception(
                f"Unknown file mode '{mode}' (must be numeric): {path}"
            )
        if (
            not exists
            or failed
            or stat.filemode(int(mode, 8)) != stat.filemode(fstat.st_mode)
        ):
            chmod_mode = mode
    if not failed:
    ...
    if exists or failed:
        if failed:
            try:
                elevation_noop(
                    attach_tty=True
                )  # avoid password prompt while writing file
                logging.debug(f"Writing to {path} using elevated /bin/tee")
                cmd: list[str] = generate_cmd_su(
                    wrap_in_bash(
                        f"tee {shell_quote(path)} >/dev/null",
                ...
                if s.returncode:
                    raise Exception(
                        f"Write command excited non-zero: {s.returncode}"
                    )
            except Exception as ex:
                logging.fatal(
                    f"Writing to file '{path}' with elevated privileges failed"
                )
                raise ex
        if chmod_mode:
...
def symlink(source, target):
    ...
    try:
        ...
        if result.returncode:
            raise Exception(
                f"Symlink creation of {target} pointing at {source} failed"
            )
