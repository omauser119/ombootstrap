...
from ombootstrap.constants import Arch, QEMU_ARCHES
from ombootstrap.exec.cmd import run_root_cmd, CompletedProcess
from ombootstrap.utils import mount
...
def is_arch_known(
    arch: Arch, raise_exception: bool = False, action: Optional[str] = None
) -> bool:
    if arch not in QEMU_ARCHES:
        if raise_exception:
            raise Exception(
                f"binfmt{f'.{action}()' if action else ''}: unknown arch {arch} (not in QEMU_ARCHES)"
            )
        return False
    return True
...
def binfmt_ensure_mounted(chroot: Optional[Chroot] = None):
    ...
    if not os.path.exists(register_path):
        ...
        result = (chroot.mount if chroot else mount)(
            "binfmt_misc", binfmt_path, options=[], fs_type="binfmt_misc"
        )  # type: ignore[operator]
        if (
            isinstance(result, CompletedProcess) and result.returncode != 0
        ) or not result:
...
def binfmt_register(arch: Arch, chroot: Optional[Chroot] = None):
    ...
    if arch == os.uname().machine:
        logging.fatal(
            "Attempted to register qemu binfmt for host architecture, skipping!"
        )
        return
    ...
    _runcmd(
        f'echo "{code}" > "{register_path}" 2>/dev/null'
    )  # use path without chroot path prefix
    if not binfmt_is_registered(arch, chroot=chroot):
        logging.debug(f"binfmt line: {code}")
        raise Exception(
            f"Failed to register qemu-user for {arch} with binfmt_misc, {binfmt_path}/{info['name']} not found"
        )
...
def binfmt_unregister(arch, chroot: Optional[Chroot] = None):
    ...
    if binfmt_is_registered(arch, chroot=chroot):
        raise Exception(
            f"Failed to UNregister qemu-user for {arch} with binfmt_misc, {chroot=}"
        )
