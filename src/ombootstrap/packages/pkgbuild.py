...
from ombootstrap.config.state import config, ConfigStateHolder
from ombootstrap.constants import Arch
from ombootstrap.distro.distro import get_kupfer_repo_names
from ombootstrap.distro.package import PackageInfo
from ombootstrap.exec.file import remove_file
from ombootstrap.logger import setup_logging
from ombootstrap.utils import git, git_get_branch
from ombootstrap.wrapper import check_programs_wrap
from ombootstrap.typehelpers import TypeAlias
...
def clone_pkgbuilds(
    ...
    if not os.path.exists(git_dir):
        ...
        if current_branch != branch:
            logging.warning(
                f"pkgbuilds repository is on the wrong branch: {current_branch}, requested: {branch}"
            )
            if switch_branch or (
                interactive
                and click.confirm(
                    "Would you like to switch branches?", default=False
                )
            ):
                ...
                result = git(
                    ["switch", *(["-f"] if discard_changes else []), branch],
                    dir=pkgbuilds_dir,
                )
                if result.returncode != 0:
                    raise Exception("failed switching PKGBUILDs branches")
            logging.warning(
                "Hint: you can use `ombs packages update` to switch branches"
            )

        if update:
            if interactive:
                if not click.confirm(
                    "Would you like to try updating the PKGBUILDs repo?",
                    default=True,
                ):
                    return
            ...
            if result.returncode != 0:
                if discard_changes:
                    logging.info(
                        "git pull failed, detecting conflicting changes"
                    )
                    # '@{u}' is a git placeholder for the latest upstream commit
                    result = git(
                        ["diff", "--name-only", "--diff-filter=UD", "@{u}"],
                        capture_output=True,
                        dir=pkgbuilds_dir,
                    )
                    ...
                    if result.stdout:
                        ...
                        for f in result.stdout.decode().split("\n"):
                            ...
                            if result.returncode != 0:
                                logging.debug(
                                    f'git checkout of file "{f}" failed; removing.'
                                )
                                remove_file(path)
...
def init_pkgbuilds(
    ...
    if repo_url == "bundled":
        from .bundled import sync_bundled

        sync_bundled(pkgbuilds_dir)
...
def get_version_specs(
    spec: str, existing_specs: Optional[VersionSpecs] = None
) -> VersionSpecs:
...
class Pkgbase(Pkgbuild):
    ...
    def __init__(
        self, relative_path: str, subpackages: list[SubPkgbuild] = [], **args
    ):
        self._built_for = set()
    ...
    def update(self, pkg: Pkgbuild):
        if not isinstance(pkg, Pkgbase):
            raise Exception(
                f"Tried to update pkgbase {self.name} with non-base pkg {pkg}"
            )
...
def parse_pkgbuild(
    ...
    if _config:
        ...
        setup_logging(
            ...
            force_colors=config.runtime.colors,
            log_setup=False,
        )  # different subprocess needs log setup.
    ...
    srcinfo_cache, lines = SrcinfoMetaFile.handle_directory(
        relative_pkg_dir, force_refresh=force_refresh_srcinfo, write=True
    )
    ...
    base_package = Pkgbase(
        relative_pkg_dir,
        sources_refreshed=sources_refreshed,
        srcinfo_cache=srcinfo_cache,
    )
    ...
    base_package.arches_hint = srcinfo_cache.build_arches_hint
        ...
        if line.startswith("pkgbase"):
            ...
            if splits[0] == "makedepends":
                if not current.makedepends:
                    current.makedepends = {}
                current.makedepends = get_version_specs(
                    spec, current.makedepends
                )

    results: list[Pkgbuild] = list(base_package.subpackages)
    if multi_pkgs:
        logging.debug(
            f" Split package detected: {base_package.name}: {results}"
        )
    ...
    for pkg in results:
        ...
        if not (pkg.version == base_package.version):
            raise Exception(
                f"Subpackage malformed! Versions differ! base: {base_package}, subpackage: {pkg}"
            )
...
def get_pkgbuild_by_path(
    ...
    if (
        lazy
        and not force_refresh_srcinfo
        and relative_path in _pkgbuilds_paths
    ):
        return _pkgbuilds_paths[relative_path]
    parsed = parse_pkgbuild(
        relative_path,
        force_refresh_srcinfo=force_refresh_srcinfo,
        _config=_config,
    )
...
def get_pkgbuild_dirs(
    quiet: bool = True, repositories: Optional[list[str]] = None
) -> list[str]:
    ...
    for repo in repositories or get_kupfer_repo_names(local=True):
        ...
        if not os.path.exists(path):
            if not quiet:
                logging.warning(
                    f'repo "{repo}" can\'t be listed: "{path}" doesn\'t exist; skipping'
                )
            continue
        for dir in os.listdir(path):
            ...
            if not os.path.exists(os.path.join(pkgbuilds_dir, p, "PKGBUILD")):
                if not quiet:
                    logging.warning(
                        f"{p} doesn't include a PKGBUILD file; skipping"
                    )
                continue
            paths.append(p)
    return paths
...
def discover_pkgbuilds(
    ...
    logging.info(
        f"Discovering PKGBUILDs{f' in repositories: {repositories}' if repositories else ''}"
    )
    ...
    if parallel:
        ...
        chunks = Parallel(
            n_jobs=multiprocessing.cpu_count() * 4, backend=backend
        )(
            delayed(get_pkgbuild_by_path)(path, lazy=lazy, _config=pass_config)
    ...
    for package in results:
        for name in [package.name] + package.replaces:
            if name in packages:
                logging.warning(
                    f"Overriding {packages[package.name]} with {package}"
                )
    ...
    for package in _pkgbuilds_cache.values():
        ...
        for dep in package.depends.copy():
            ...
            for pkg in _pkgbuilds_cache.values():
                ...
                if dep in pkg.names():
                    logging.debug(
                        f"{package.path}: Found {pkg.name} that provides {dep}"
                    )
                    found = True
                    break
            if not found:
                logging.debug(
                    f"{package.path}: Removing {dep} from local dependencies"
                )
...
def filter_pkgbuilds(
    ...
    if not (use_names or use_paths):
        raise Exception(
            "Error: filter_packages instructed to match neither by names nor paths; impossible!"
        )
    paths = list(paths)
    ...
    if "all" in paths:
        ...
        if arch:
            all_pkgs = [
                pkg
                for pkg in all_pkgs
                if set([arch, "any"]).intersection(pkg.arches)
            ]
        return all_pkgs
    result = []
    to_find = list(paths)
    for pkg in repo.values():
        ...
        if matches:
            ...
            if arch and not set([arch, "any"]).intersection(pkg.arches):
                logging.warn(
                    f"Pkg {pkg.name} matches query {matches[0]} but isn't available for architecture {arch}: {pkg.arches}"
                )
                continue
            result += [pkg]
    ...
    if not allow_empty_results:
        if not result:
            raise Exception(
                f"No packages matched by {fields_err}: "
                + ", ".join([f'"{p}"' for p in paths])
            )
        if to_find:
            raise Exception(
                f"No packagages matched by {fields_err}: "
                + ", ".join([f'"{p}"' for p in to_find])
            )
