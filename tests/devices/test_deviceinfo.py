...
from ombootstrap.devices.deviceinfo import DeviceInfo, parse_deviceinfo
from ombootstrap.devices.device import get_device
...
def test_parse_variant_deviceinfo():
    ...
    text = deviceinfo_text + "\n".join(
        [""]
        + [
            f"deviceinfo_{field}_{variant}={value}"
...
def test_get_variant_deviceinfo_from_repo():
    ...
    assert (
        "dtb" in info
    )  # variant-specific variable, check it has been stripped down from 'dtb_ebbg' to 'dtb'
